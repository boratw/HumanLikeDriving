        ...
        for exp in [input_exp]:
            for dangerous in [False]:
                try:
                    ...
                    if dangerous:
                        distance_to_leading_vehicle = [ np.random.uniform(10.0, 15.0) for i in range(agent_num) ]
                        ...
                        desired_velocity = [ 11.1111 * (1.0 - vehicle_speed[i] / 100.0)  for i in range(agent_num) ]
                        steering_ratio = [ np.clip(np.random.normal(1., 0.1), 0.8, 1.2) for i in range(agent_num) ]
                    else:
                    ...
                    log_file = open("policy_test_log/result_1128/sff_" + ("d" if dangerous else "s") + "_policy_" + str(exp) + ".txt", "wt")
                    log_file_other = open("policy_test_log/result_1128/sff_" + ("d" if dangerous else "s") + "_other_" + str(exp) + ".txt", "wt")
                    ...
                    if exp % 5 == 0:
                        sff.Assign_Player(actor.player, record_video_name = "policy_test_log/result_1128/sff_" + ("d" if dangerous else "s") + "_policy_" + str(exp) + ".avi")
                    else:
                    ...
                    for step in range(20000):
                        ...
                        log_file.write(str(step + 1) + "\t" + str(moved) + "\t" + str(tr.location.x) + "\t" + str(tr.location.y) + "\t" + str(tr.rotation.yaw) + "\t"
                                    + str(v) + "\t" + str(target_velocity) + "\t" + str(accel) + "\t" + str(brake) + "\t" + str(steer)  + "\t" + log + "\n")
