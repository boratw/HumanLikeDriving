...
log_file = open("train_log/DrivingStyle8/log_" + log_name + ".txt", "wt")
        ...
        for epoch in range(1, 10000):
            ...
            for iter in range(len(history) * 8):
                ...
                for step in range(max_step):
                    ...
                    for x in range(agent_num):
                        if cur_history[x][step][0]:
                            global_latent[x] += mu[i] * np.exp(-var[i])
                            global_latent_sum[x] += np.exp(-var[i])
                            i += 1
                ...
                for step in range(max_step):
                    ...
                    learner.optimize_decoder(epoch, state_dic, nextstate_dic, route_dic, action_dic, global_latent_dic)
            ...
            if epoch % 10 == 0:
                learner_saver.save(sess, "train_log/DrivingStyle8/log_" + log_name + "_" + str(epoch) + ".ckpt")
