...
try:
    ...
    # Spawn vehicles
    # --------------
    for impatient_lane_change_mag in [1, 2, 4, 8]:
        for steer_ratio_mag in [1, 2, 4, 8]:
            for vel_noise_mag in [1, 2, 4, 8]:
                for lane_noise_mag in [1, 2, 4, 8]:
                    save_objs = []
                    ...
                    steering_ratio = [ np.clip(np.random.normal(0., 0.025), -0.05, 0.05) * steer_ratio_mag + 1.0 for i in range(agent_num) ]

                    desired_velocity = [ 11.1111 * (1.0 - vehicle_speed[i] / 100.0)  for i in range(agent_num) ]
                    vel_disp = [ np.random.uniform(0.0000001, 0.005 * vel_noise_mag) for i in range(agent_num) ]
                    lane_disp = [ np.random.uniform(0.0000001, 0.00003 * lane_noise_mag) for i in range(agent_num) ]

                    impatient_lane_change = [ np.clip(np.random.normal(6.0, 4.0), -2.0, 20.0) * impatient_lane_change_mag + 20.0  for i in range(agent_num) ]
                    
                    exp_str = "_LC" + str(impatient_lane_change_mag) +  "_SR" + str(steer_ratio_mag) + \
                            "_VN" + str(vel_noise_mag) +  "_LN" + str(lane_noise_mag)
                    for iteration in range(20):
                        ...
                        for step in range(2000):
                            ...
                            for i, actor in enumerate(all_vehicle_actors):
                                ...
                                vel_add[i] = vel_add[i] * 0.96 + vel_add_diff[i]
                                lane_add[i] = lane_add[i] * 0.96 + lane_add_diff[i]

                                vel_add_diff[i] += np.random.normal( -vel_add_diff[i] * vel_disp[i], vel_disp[i])
                                lane_add_diff[i] += np.random.normal( -lane_add_diff[i] * lane_disp[i], lane_disp[i])
                    ...
                    with open("data/gathered_from_npc2/data_" + exp_str + ".pkl","wb") as fw:
