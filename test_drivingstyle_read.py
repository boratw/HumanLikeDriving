...
with sess.as_default():
    if module_n == 0:
        learner = DrivingStyleLearner(state_len=state_len, prevstate_len=prevstate_len, nextstate_len=nextstate_len, 
                                       isTraining=False, test_mask_use = False)
    elif module_n == 1:
        learner = DrivingStyleLearner(state_len=state_len, prevstate_len=prevstate_len, nextstate_len=nextstate_len, 
                                       isTraining=False, test_mask_use = True, test_zero_latent=True)
    elif module_n == 2:
        learner = DrivingStyleLearner(state_len=state_len, prevstate_len=prevstate_len, nextstate_len=nextstate_len, 
                                       isTraining=False, test_mask_use = True, test_input_latent=True)
    elif module_n == 3:
        learner = DrivingStyleLearner(state_len=state_len, prevstate_len=prevstate_len, nextstate_len=nextstate_len, 
                                       isTraining=False, test_mask_use = True)
    learner_saver = tf.train.Saver(var_list=learner.trainable_dict, max_to_keep=0)
    learner_saver.restore(sess, "train_log/DrivingStyle_AvgLatent4/2024-03-13-14-05-16_100.ckpt")

    for pkl_index in range(10):
        ...
        for exp_index in range(10):
            ...
            if module_n == 2:
                for step in range(step_count // 5):
                    ...
                    for x in range(agent_count):
                        ...
                        prevstate_dic.append(cur_history[x][step][1])
                        nextstate_dic.append(cur_history[x][step][2])

                    res_mu, res_std = learner.get_latent(state_dic, prevstate_dic, nextstate_dic, discrete=True)
                    for x in range(agent_count):
                        global_latent += res_mu[x] * (res_std ** 2)
                        global_latent_sum += (res_std ** 2)
            ...
            for step in range(step_count // 5):
                ...
                res_route, res_action, _ = learner.get_output(state_dic, global_latent, prevstate_dic, nextstate_dic, discrete=True)
