...
def SendLatentOutput(list):
    ...
    print(cur_history[target][current_step][0][4:24])
    print(lane_tracers[target].Trace(state_vectors[current_step][target][0], state_vectors[current_step][target][1]))
...
with sess.as_default():
    ...
    learner_saver.restore(sess, "train_log/DrivingStyle5/log_17-05-2023-15-50-00/log_17-05-2023-15-50-00_1500.ckpt")
            ...
            if traced == None:
                ...
                if len(state_vectors[step][i][8]) >= 1:
                    ...
                    for trace, c in zip(traced, tracec):
                        if c:
                            dist = (trace[0][0] - state_vectors[step][i][8][0][1]) ** 2 + (trace[0][1] - state_vectors[step][i][8][0][2]) ** 2
                            if dist < mindist:
                                trace_result = trace
                                mindist = dist
    ...
    for step in range(0, step_count):
        ...
        res_mu, res_sig = learner.get_latent(state_dic, nextstate_dic)
        for x in range(agent_count):
