...
def SendLatents(list):
    res = json.dumps({"mu" : [0, 0, 0, 0]}, cls=MyEncoder)
    return res
...
def SendOutput(list):
    ...
    state_dic = [cur_history[target][current_step - step_start_index][0] for _ in range(pred_num)]
    action_dic = [i for i in range(pred_num)]
    
    with sess.as_default():
        l_state, l_prob = learner.get_output(state_dic, action_dic, discrete=False)
        
    l_prob = np.mean(l_prob, axis=0)
    res = json.dumps({"route" : d_state, "predicted" : l_state, "action_prob" : l_prob}, cls=MyEncoder)
...
sess = tf.Session()
with sess.as_default():
    learner = DrivingStyleLearner(state_len=state_len, nextstate_len=nextstate_len, isTraining=False)
    learner_saver = tf.train.Saver(var_list=learner.trainable_dict, max_to_keep=0)
    learner_saver.restore(sess, "train_log/DrivingStyle_NoLatent2/log_2023-12-06-09-01-39_420.ckpt")
