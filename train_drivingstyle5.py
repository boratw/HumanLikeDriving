...
from network.DrivingStyle5 import DrivingStyleLearner
...
log_file = open("train_log/DrivingStyle5/log_" + log_name + ".txt", "wt")
...
def parallel_task(item):
    ...
    for step in range(stepstart, len(state_vectors)-150, 4):
        for i in range(agent_count):
            if torque_added[i] == 0:
                if state_vectors[step+20][i][9] != 0 or state_vectors[step+21][i][9] != 0 or state_vectors[step+22][i][9] != 0 or state_vectors[step+23][i][9] != 0:
                    torque_added[i] = 5
                else:
                    other_vcs = []
                    ...
                    if (relposx * relposx + relposy * relposy) > 0.01 or random.random() < 0.1:
                        ...
                        if traced != None:
                            ...
                            if len(route) == 10:
                                history_exp[i].append( [np.concatenate([[velocity, (1. if state_vectors[step][i][5] == 0. else 0.), px, py], waypoints, other_vcs[:8,:6].flatten()]), route])
            else:
                torque_added[i] -= 1
    history = []
    ...
    with multiprocessing.Pool(processes=20) as pool:
        ...
        for epoch in range(1, 10000):
            ...
            for iter in range(len(history) * 32):
                ...
                step_dic = [ random.choices(list(range(len(cur_history[x]))), k = 64) for x in agent_dic ]
            ...
            if epoch % 50 == 0:
                learner_saver.save(sess, "train_log/DrivingStyle5/log_" + log_name + "_" + str(epoch) + ".ckpt")
