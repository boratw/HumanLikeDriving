...
def SendOutput(list):
    ...
    for j in range(0, 80, 20):
        if current_step + j < len(state_vectors):
            d_state.append([state_vectors[current_step + j][target][0], state_vectors[current_step + j][target][1]])
        else:
            d_state.append([state_vectors[-1][target][0], state_vectors[-1][target][1]])
...
with sess.as_default():
    ...
    with open("policy_test_log/result_240220/exp_149_DriveStyle.pkl","rb") as fr:
        data = pickle.load(fr)


    state_vectors = data["state_vectors"]
    control_vectors = data["control_vectors"]
    param_vectors = data["params"]
    ...
    step_start_index = 20
    step_count = len(state_vectors) - step_start_index
    ...
    for step in range(step_start_index, step_start_index+step_count):
        ...
        for i in range(agent_count):
            ...
            if state_vectors[step][i][5] == 0:
                if (px * px + py * py) >  (( state_vectors[step][i][6][0] - x) * ( state_vectors[step][i][6][0] - x) \
                                            + ( state_vectors[step][i][6][1] - y) * ( state_vectors[step][i][6][1] - y)):
                    px, py = rotate( state_vectors[step][i][6][0] - x,  state_vectors[step][i][6][1] - y, yawsin, yawcos)
