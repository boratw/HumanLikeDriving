...
from network.DrivingStyle3_bayesian_latent import DrivingStyleLearner
...
log_name = "train_log/DrivingStyle3_Bayesian_Latent/log_" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            ...
            for iter in range(len(history) * 8):
                ...
                for agent_dic_it in agent_dic:
                    ...
                    for x in agent_dic_it:
                        ...
                        dic_num.append(len(state_dic) - prev_len)

                    if len(state_dic) > 0:
                        mu, var = learner.optimize_local(state_dic, nextstate_dic, route_dic)
                        ...
                        for x in dic_num:
                            it_mid = it_start + x // 2
                            it_end = it_start + x
                            m = np.sum(mu[it_start:it_mid] * np.exp(-var[it_start:it_mid] - 4.), axis=0)
                            s = np.sum(np.exp(-var[it_start:it_mid] - 4.), axis=0)
                            latent_dic.extend([m / s] * (it_end - it_start))
                            it_start += x
                        ...
                        for x in dic_num:
                            it_mid = it_start + x // 2
                            it_end = it_start + x
                            m = np.sum(mu[it_mid:it_end] * np.exp(-var[it_mid:it_end] - 4.), axis=0)
                            s = np.sum(np.exp(-var[it_mid:it_end] - 4.), axis=0)
                            latent_dic.extend([m / s] * (it_end - it_start))
                            it_start += x
                        ...
                        for x in dic_num:
                            it_end = it_start + x
                            m = np.sum(mu[it_start:it_end] * np.exp(-var[it_start:it_end] - 4.), axis=0)
                            s = np.sum(np.exp(-var[it_start:it_end] - 4.), axis=0)
                            latent_dic.extend([m / s] * (it_end - it_start))
                            it_start += x

                        state_dic = state_dic * 3
                        nextstate_dic = nextstate_dic * 3
                        route_dic = route_dic * 3
                        learner.optimize_global(state_dic, nextstate_dic, route_dic, latent_dic)
