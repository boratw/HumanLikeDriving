...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, nextstate_len = 2, route_len = 10, action_len=3, latent_len=4, regularizer_weight= 0.0001,
                 discrete_lr = 0.001, discrete_action_norm_lr = 0.0001, latent_lr = 0.001, shuffled_lr=0.1, istraining=True,
            ...
            if istraining:
                with tf.variable_scope("LatentLearner", reuse=reuse):
                    ...
                    for output_latent, output_latent_var in zip(output_latents, output_latent_vars):
                        latent_var = tf.reduce_mean(output_latent_var, axis=1)
                        ...
                        shuffled_latent1.append(tf.gather(output_latent, indicies1, axis=0, batch_dims=0))
                        indicies2 = tf.random.shuffle(tf.tile(sorted_indicies[:indicies_num // 4], [4]))
                        shuffled_latent2.append(tf.gather(output_latent, indicies2, axis=0, batch_dims=0))
                        indicies3 = tf.random.shuffle(tf.tile(sorted_indicies[:indicies_num // 8], [8]))
                        shuffled_latent3.append(tf.gather(output_latent, indicies3, axis=0, batch_dims=0))

                    shuffled_latent1 = tf.concat(shuffled_latent1, axis=0)
                    shuffled_latent2 = tf.concat(shuffled_latent2, axis=0)
                    shuffled_latent3 = tf.concat(shuffled_latent3, axis=0)
                    ...
                    self.ls_route_error = tf.abs(self.ls_output_diff - self.ls_batched_route_diff)
                    ...
                    self.ls_mu_loss = tf.reduce_mean(self.ls_latent.mu ** 2)
