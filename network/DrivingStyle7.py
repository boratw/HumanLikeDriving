...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, agent_for_each_train=8, state_len = 59, route_len = 10, nextstate_len=2, action_len=3, global_latent_len = 4,
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.global_encoder = VAE_Encoder(nextstate_len + state_len + action_len * route_len, global_latent_len, [256, 256, 256], hidden_nonlns = tf.nn.leaky_relu, 
            ...
            self.decoder_input = tf.concat([self.global_encoder.layer_output, self.layer_input_state, layer_input_route_selected], axis=1)
            self.route_decoder = MLP(global_latent_len + state_len + route_len, nextstate_len, [256, 256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=self.decoder_input, name="RouteDecoder", use_dropout=False  )
            self.action_decoder = MLP(global_latent_len + state_len + route_len, action_len, [256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=self.decoder_input, name="ActionDecoder", use_dropout=False  )
            ...
            latent_decoder_input = tf.concat([self.layer_input_global_latent, self.layer_input_state, layer_input_route_selected], axis=1)
            self.latent_route_decoder = MLP(global_latent_len + state_len + route_len, nextstate_len, [256, 256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=latent_decoder_input, name="RouteDecoder", reuse=True, use_dropout=False )
            self.latent_route_decoder_output = self.latent_route_decoder.layer_output

            self.latent_action_decoder = MLP(global_latent_len + state_len + route_len, action_len, [256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=latent_decoder_input, name="ActionDecoder", reuse=True, use_dropout=False )
            self.latent_action_decoder_output = tf.nn.softmax(self.latent_action_decoder.layer_output)
    ...
    def get_global_decoded(self, input_state, input_global_latent):
        input_list = {self.layer_input_state : input_state, self.layer_input_global_latent : input_global_latent, self.layer_dropout : 0.0}
        sess = tf.get_default_session()
        l1, l2 = sess.run([self.latent_route_decoder_output, self.latent_action_decoder_output], input_list)
        return l1, l2
