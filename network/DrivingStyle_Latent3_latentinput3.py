...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, prevstate_len = 2, nextstate_len = 2, action_len=31, latent_len=4, regularizer_weight= 0.0001,
                 learner_lr = 0.001,  normalizer_lr = 0.01, sigma_bias=-1.0, isTraining=True, state_feature=128, route_loss_weight=1.0, 
                 action_loss_weight=1.0, avgmask_loss_weight=1.0, test_mask_use=True):
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            learner_lr = tf.train.cosine_decay(learner_lr, self.layer_input_iter, 100)
            normalizer_lr = tf.train.exponential_decay(normalizer_lr, self.layer_input_iter, 20, 0.5)
            ...
            mask_input = tf.concat([self.state_normalized, self.layer_input_prevstate, self.layer_input_nextstate], axis=1)
            self.mask_enc1 = FC(mask_input, state_len + prevstate_len + nextstate_len, 64, input_dropout = self.layer_input_dropout, 
            ...
            self.mask_enc2 = Variational_FC(self.mask_enc1.layer_output, 64, latent_len, input_dropout = None, 
                                    output_nonln = None, name="mask_enc2", sigma_bias=-3)
            ...
            self.mask_output = tf.math.softplus(self.mask_output_raw)
            #self.mask_output = self.mask_output_raw / tf.sqrt(tf.reduce_mean(self.mask_output_raw ** 2, axis=1, keepdims=True))
            
            use_latent_onehot = tf.one_hot(tf.random.uniform([tf.shape(self.layer_input_latent)[0]], 0, latent_len, tf.dtypes.int32), latent_len)
            use_latent = self.layer_input_latent * use_latent_onehot + self.mask_latent * (1. - use_latent_onehot)
            self.latent_mask_dec1 = FC(use_latent, latent_len, 64, input_dropout = self.layer_input_dropout, 
                                    output_nonln = tf.nn.leaky_relu, name="mask_dec1", reuse=True)
            self.latent_mask_dec2 = FC(self.latent_mask_dec1.layer_output, 64, state_len, input_dropout = None, 
                                    output_nonln = None, name="mask_dec2", reuse=True)
            
            self.latent_mask_output_raw = self.latent_mask_dec2.layer_output
            self.latent_mask_output = tf.math.softplus(self.latent_mask_output_raw)
            #self.latent_mask_output = self.latent_mask_output_raw / tf.sqrt(tf.reduce_mean(self.latent_mask_output_raw ** 2, axis=1, keepdims=True))
            ...
            ## No latent masked
            self.state_output_i = self.state_i4
            self.state_output = tf.reduce_sum(self.state_output_i, axis=1) / state_len
            ...
            ## Avg latent masked
            self.avgmask_state_output_i = self.state_i4 * tf.reshape(self.latent_mask_output, [-1, state_len, 1])
            self.avgmask_state_output = tf.reduce_sum(self.avgmask_state_output_i, axis=1) / state_len
            
            self.avgmask_action_h1 = FC(self.avgmask_state_output, state_feature, 256, input_dropout = self.layer_input_dropout, 
                                    output_nonln = tf.nn.leaky_relu, name="action_h1", reuse=True)
            self.avgmask_action_h2 = FC(self.avgmask_action_h1.layer_output, 256, action_len, input_dropout = None, 
                                    output_nonln = None, name="action_output", reuse=True)
            self.avgmask_action_output_raw = self.avgmask_action_h2.layer_output
            self.avgmask_action_output = tf.nn.softmax(self.avgmask_action_output_raw)
                            
            avgmask_route_input = tf.concat([self.avgmask_state_output, route_input_action], axis=1)
            self.avgmask_route_h1 = FC(avgmask_route_input, state_feature + 1, 512, input_dropout = self.layer_input_dropout, 
                                output_nonln = tf.nn.leaky_relu, name="route_h1", reuse=True)
            self.avgmask_route_h2 = FC(self.avgmask_route_h1.layer_output, 512, 256, input_dropout = self.layer_input_dropout, 
                                output_nonln = tf.nn.leaky_relu, name="route_h2", reuse=True)
            self.avgmask_route_h3 = FC(self.avgmask_route_h2.layer_output, 256, nextstate_len, input_dropout = None, 
                                output_nonln = None, name="route_h3", reuse=True)
            self.avgmask_route_output = self.avgmask_route_h3.layer_output


            ## masked
            self.masked_state_output_i = self.state_i4 * tf.reshape(self.mask_output * 0.9 + self.latent_mask_output + 0.1, [-1, state_len, 1]) 
            ...
            if isTraining:
                ...
                self.avgmask_action_error = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(action_label, self.avgmask_action_output_raw))
                self.avgmask_route_error = tf.reduce_mean(tf.abs(self.avgmask_route_output - self.layer_input_nextstate), axis=0)
                ...
                predict_train_vars = [*self.action_h1.trainable_params, *self.action_h2.trainable_params,
                ...
                self.predict_train = self.predict_optimizer.minimize(loss = 
                                        ...
                                        + (self.route_reg_loss + self.action_reg_loss) * regularizer_weight, 
                                        var_list = predict_train_vars)
                

                self.mask_reg_loss = tf.reduce_sum([ g.regularization_loss for g in 
                                                [self.mask_enc1, self.mask_enc2, self.latent_mask_dec1, self.latent_mask_dec2]])
                ...
                mask_train_vars = [*self.mask_enc1.trainable_params, *self.mask_enc2.trainable_params, 
                                   *self.latent_mask_dec1.trainable_params, *self.latent_mask_dec2.trainable_params ]
                self.mask_train = self.mask_optimizer.minimize(loss = 
                                        tf.reduce_mean(self.masked_action_error * route_loss_weight) 
                                        + self.masked_route_error * action_loss_weight
                ...
                if test_mask_use:
                    self.final_state_output_i = self.state_i4 * tf.reshape(self.final_mask_output, [-1, state_len, 1])  
                    self.final_state_output = tf.reduce_sum(self.final_state_output_i, axis=1) / state_len
                else:
                    self.final_state_output_i = self.state_i4
                    self.final_state_output = tf.reduce_sum(self.final_state_output_i, axis=1) / state_len
                ...
                action_constant = tf.constant(list(range(action_len)), dtype=tf.float32, shape=[action_len, 1])
    ...
    def network_initialize(self):
        ...
        self.log_avg_route_rec = np.array([0.] * self.nextstate_len)
        self.log_avg_action_rec = 0.
    ...
    def optimize(self, input_iter, input_state, input_prevstate, input_nextstate, input_action, input_latent):
        input_list = {self.layer_input_state : input_state, self.layer_input_prevstate: input_prevstate, self.layer_input_nextstate: input_nextstate, 
                      self.layer_input_action : input_action, self.layer_input_latent : input_latent, self.layer_input_dropout : 0.05,
                      self.layer_input_iter : input_iter}
        sess = tf.get_default_session()
        _, le1, le2, lr3, lr4, lr2 = sess.run([self.predict_train,
                                    self.action_error, self.route_error, 
                                    self.action_reg_loss, self.route_reg_loss, self.state_reg_loss],input_list)
        _, le3, le4, le5, le6, ll1, ll2, lm1, lm2, lr1 = sess.run([self.mask_train,
                                    self.masked_action_error, self.masked_route_error, self.avgmask_action_error, self.avgmask_route_error,
        ...
        self.log_avg_action_rec += le5
        self.log_avg_route_rec += le6
    ...
    def get_output(self, input_state, input_latent, discrete=True):
        ...
        sess = tf.get_default_session()
        l1, l2, l3 = sess.run([self.final_route_output, self.final_action_output, self.latent_mask_output] , input_list)
        return l1, l2, l3
    
    def get_latent(self, input_state, input_prevstate, input_nextstate, discrete=True):
        input_list = {self.layer_input_state : input_state, self.layer_input_prevstate: input_prevstate, 
                      self.layer_input_nextstate : input_nextstate,
                      self.layer_input_dropout : (0.0 if discrete else 0.1)}
        sess = tf.get_default_session()
        l1, l2 = sess.run([self.mask_enc2.mu, self.mask_enc2.sig], input_list)
        return l1, l2
    
    def get_mask(self, input_state, input_latent, discrete=True):
        input_list = {self.layer_input_state : input_state, self.layer_input_latent : input_latent,
                      self.layer_input_dropout : (0.0 if discrete else 0.1)}
        sess = tf.get_default_session()
        l1 = sess.run(self.latent_mask_output , input_list)
        return l1
    ...
    def log_caption(self):
        return "\t" \
            ...
            + self.name + "_AvgRouteReconLoss\t" + "\t".join([ "" for _ in range(self.nextstate_len)]) \
            + self.name + "_AvgActionReconLoss\t" \
    ...
    def current_log(self):
        ...
        return "\t" \
            ...
            + "\t".join([str(self.log_avg_route_rec[i] / log_num) for i in range(self.nextstate_len)]) + "\t"\
            + str(self.log_avg_action_rec / log_num) + "\t"\
    ...
    def log_print(self):
        ...
        print ( self.name \
            ...
            + "\n\tAvgRouteReconLoss    : " + " ".join([str(self.log_avg_route_rec[i] / log_num)[:8] for i in range(self.nextstate_len)])\
            + "\n\tAvgActionReconLoss   : "  + str(self.log_avg_action_rec / log_num) \
