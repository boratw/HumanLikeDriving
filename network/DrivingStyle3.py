...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, agent_for_each_train=16, state_len = 59, nextstate_len=2, global_latent_len = 4, 
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.global_decoder = MLP(global_latent_len + state_len, nextstate_len, [256, 256], hidden_nonlns = tf.nn.elu, 
                        input_tensor=self.global_decoder_input, name="GlobalDecoder", use_dropout=True, input_dropout=self.layer_dropout  )

            global_latent_decoder_input = tf.concat([self.layer_input_global_latent, self.layer_input_state], axis=1)
            self.global_latent_decoder = MLP(global_latent_len + state_len, nextstate_len, [256, 256], hidden_nonlns = tf.nn.elu, 
                        input_tensor=global_latent_decoder_input, name="GlobalDecoder", reuse=True, use_dropout=True, input_dropout=self.layer_dropout )
