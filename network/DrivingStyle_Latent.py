...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, prevstate_len = 2, nextstate_len = 2, action_len=31, latent_len=4, regularizer_weight= 0.0001,
                 learner_lr = 0.001,  sigma_bias=-1.0, isTraining=True, state_feature=64):
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.mask_output_i = tf.math.sigmoid(tf.clip_by_value(self.mask_output_raw, -10.0, 10.0))
            self.mask_output = self.mask_output_i / tf.stop_gradient(tf.reduce_sum(self.mask_output_i, axis=1, keepdims=True))
            ...
            ## No Mask
            self.state_output = tf.reduce_sum(self.state_i4, axis=1) * (1.5 / state_len)
            ...
            ## Independent
            self.ind_state_output_i = self.state_i4 * 1.5
            self.ind_state_output = tf.reshape(self.ind_state_output_i, [-1, state_feature])
            ...
            self.ind_action_output_raw = self.ind_action_h2.layer_output
            self.ind_action_output = tf.nn.softmax(self.ind_action_output_raw)
            ...
            ## mask
            self.masked_state_output_i = self.state_i4 * tf.reshape(self.mask_output + (0.5 / state_len), [-1, state_len, 1])
            self.masked_state_output = tf.reduce_sum(self.masked_state_output_i, axis=1)
            ...
            if isTraining:
                ...
                ind_action_label = tf.reshape(tf.tile(tf.reshape(action_label, [-1, 1]), [1, state_len]), [-1])
                self.ind_action_error = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(ind_action_label, self.ind_action_output_raw))
                ind_input_nextstate = tf.reshape(tf.tile(self.layer_input_nextstate, [1, state_len]), [-1, nextstate_len])
                self.ind_route_error = tf.reduce_mean((self.ind_route_output - ind_input_nextstate) ** 2, axis=0)
                ...
                self.masked_route_error = tf.reduce_mean((self.masked_route_output - self.layer_input_nextstate) ** 2, axis=0)
                ...
                self.predict_train_gradient = self.predict_optimizer.compute_gradients(loss = tf.reduce_mean(self.route_error) + self.action_error
                                                                + tf.reduce_mean(self.ind_route_error) + self.ind_action_error
                                                                + (self.state_reg_loss + self.route_reg_loss + self.action_reg_loss) * regularizer_weight, 
                                                                var_list = predict_train_vars)
                predict_train_gradient_clipped = [ ( None if g is None else tf.clip_by_value(g, -0.1, 0.1), v) for g, v in self.predict_train_gradient]
                self.predict_train = self.predict_optimizer.apply_gradients(predict_train_gradient_clipped)
                ...
                self.mask_train_gradient = self.mask_optimizer.compute_gradients(loss =  tf.reduce_mean(self.masked_route_error) + self.masked_action_error
                                                                + self.mask_reg_loss * regularizer_weight, 
                                                                var_list = mask_train_vars)
                mask_train_gradient_clipped = [ ( None if g is None else tf.clip_by_value(g, -0.1, 0.1), v) for g, v in self.mask_train_gradient]
                self.mask_train = self.mask_optimizer.apply_gradients(mask_train_gradient_clipped)
    ...
    def optimize(self, input_state, input_prevstate, input_nextstate, input_action):
        ...
        _, le1, le2, le5, le6, lr2, lr3, lr4, = sess.run([self.predict_train,
                                    self.action_error, self.route_error, self.ind_action_error, self.ind_route_error,
                                    self.state_reg_loss, self.action_reg_loss, self.route_reg_loss],input_list)
