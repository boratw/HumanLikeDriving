...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, agent_for_each_train=8, state_len = 59, nextstate_len=2, global_latent_len = 4,
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.encoder_reconstruction_loss = tf.reduce_mean(reconstruction_loss_batch[0], axis=0) + tf.reduce_mean(reconstruction_loss_batch[1:], axis=[0, 1])
            ...
            self.decoder_reconstruction_loss = tf.reduce_mean(tf.abs(self.layer_input_nextstate - self.decoder.layer_output), axis=0)
