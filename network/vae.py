...
class VAE:
    def __init__(self, input_dim, latent_dim, hidden_dims, hidden_nonlns=tf.nn.relu,
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            if additional_dim != None:
                layer_latent_decoder_input = tf.concat([self.layer_latent, self.layer_additional], axis=1)
            else:
                layer_latent_decoder_input = self.x
            self.latent_decoder = MLP(decoder_input_dim, input_dim, hidden_dims[::-1], hidden_nonlns,
                               input_tensor=layer_latent_decoder_input, use_dropout=False, name="Dec", reuse=True)
