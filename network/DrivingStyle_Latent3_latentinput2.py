...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, prevstate_len = 2, nextstate_len = 2, action_len=31, latent_len=4, regularizer_weight= 0.0001,
                 learner_lr = 0.001,  sigma_bias=-1.0, isTraining=True, state_feature=128, route_loss_weight=1.0, 
                 action_loss_weight=1.0, avgmask_loss_weight=1.0, test_mask_use=True, test_input_latent=False, test_zero_latent=False):
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.latent_mask_output = tf.exp(tf.clip_by_value(self.latent_mask_output_raw, -5.0, 5.0))
            #self.latent_mask_output = self.latent_mask_output_i / tf.reduce_mean(self.latent_mask_output_i, axis=1, keepdims=True)
            ...
            ## No latent masked
            self.state_output_i = self.state_i4 * tf.reshape(self.default_mask_normalized, [1, -1, 1])
            ...
            ## Avg latent masked
            self.avgmask_state_output_i = self.state_i4 * tf.reshape(self.latent_mask_output, [-1, state_len, 1]) * tf.reshape(self.default_mask_normalized, [1, -1, 1])
            ...
            if isTraining:
                ...
                self.mask_train = self.mask_optimizer.minimize(loss = 
                                        tf.reduce_mean(self.avgmask_route_error * route_loss_weight) 
                                        + self.avgmask_action_error * action_loss_weight
                ...
                if test_mask_use:
                    if test_input_latent:
                        ## use latent input
                        self.final_mask_dec1 = FC(self.layer_input_latent, latent_len, 64, input_dropout = self.layer_input_dropout, 
                                                output_nonln = tf.nn.leaky_relu, name="mask_dec1", reuse=True)
                    elif test_zero_latent:
                        ## use zero input
                        self.final_mask_dec1 = FC(self.layer_input_latent * 0., latent_len, 64, input_dropout = self.layer_input_dropout, 
                                                output_nonln = tf.nn.leaky_relu, name="mask_dec1", reuse=True)
                    else:
                        ## use latent estimate input
                        self.final_mask_dec1 = FC(self.mask_latent, latent_len, 64, input_dropout = self.layer_input_dropout, 
                                                output_nonln = tf.nn.leaky_relu, name="mask_dec1", reuse=True)
    ...
    def optimize(self, input_state, input_prevstate, input_nextstate, input_action, input_latent):
        ...
        _, lm3, le1, le2, le5, le6, lr3, lr4, lr2 = sess.run([self.predict_train,
                                    self.default_mask_normalized, self.action_error, self.route_error, self.avgmask_action_error, self.avgmask_route_error,
    ...
    def get_output(self, input_state, input_latent, input_prevstate=None, input_nextstate = None, discrete=True):
        ...
        if input_prevstate != None:
            input_list[self.layer_input_prevstate] = input_prevstate
        if input_nextstate != None:
            input_list[self.layer_input_nextstate] = input_nextstate
        sess = tf.get_default_session()
