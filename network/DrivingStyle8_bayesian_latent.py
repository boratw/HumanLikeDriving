...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, nextstate_len = 2, route_len = 10, action_len=3, latent_len=4, regularizer_weight= 0.0001,
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            with tf.variable_scope("DiscreteLearner", reuse=reuse):
                ...
                grads =  self.ds_optimizer.compute_gradients(loss = self.ds_min_route_loss, var_list=train_route_vars )
                new_grads = []
                for grad, v in grads:
                    new_grads.append((None if grad == None else tf.clip_by_value(grad, -1.0, 1.0), v))
                self.ds_train_route = self.ds_optimizer.apply_gradients(new_grads)
            ...
            if istraining:
                with tf.variable_scope("LatentLearner", reuse=reuse):
                    ...
                    self.ls_action = Variational_FC(self.ls_dec_h1.layer_output, 256, 1, input_dropout = None, 
                                        output_nonln = tf.math.tanh, name="ls_action")
                    ...
                    self.ls_output_action = self.ls_action.layer_output
                    ...
                    self.ls_route_loss = tf.reduce_mean((self.ls_output_diff - self.dsc_min_route_diff) ** 2)
                    action_target = self.action_label[:, 1] - self.action_label[:, 2]
                    self.ls_action_loss = tf.reduce_mean((self.ls_output_action - tf.reshape(action_target, [-1, 1])) ** 2)
                    ...
                    self.ls_dec_reg_loss = (self.ls_dec_h1.regularization_loss + self.ls_dec_h2.regularization_loss+
                                            self.ls_diff.regularization_loss + self.ls_action.regularization_loss) / 4
                    ...
                    self.ls_average_action_var = tf.reduce_mean(self.ls_action.var)
                    ...
                    grads =  self.ls_optimizer.compute_gradients(loss = 
                                                            self.ls_route_loss + self.ls_action_loss +
                    ...
                    for grad, v in grads:
                        new_grads.append((None if grad == None else tf.clip_by_value(grad, -1.0, 1.0), v))
                    self.ls_train_latent = self.ls_optimizer.apply_gradients(new_grads)
                ...
                with tf.variable_scope("LatentLearner", reuse=reuse):
                    ...
                    self.ls_infer_action = Variational_FC(self.ls_infer_dec_h1.layer_output, 256, 1, input_dropout = None, 
                                        output_nonln = tf.math.tanh, name="ls_action", reuse=True)
                    ...
                    self.output_route_mean = self.dsc_output_route - tf.reshape(self.ls_infer_diff.mu, [-1, 1, nextstate_len])
                    self.output_route_var = tf.reshape(tf.tile(self.ls_infer_diff.var, [1, action_len]), [-1, action_len, nextstate_len])
                    self.output_action_mean = tf.reshape(self.ls_infer_action.mu, [-1])
                    self.output_action_var = tf.reshape(self.ls_infer_action.var, [-1])
    ...
    def optimize(self, input_state, input_nextstate, input_route, input_action, input_index):
        ...
        _, l1, l2, l10, l3, l4, l5, l9, l7, l11 =\
                sess.run([self.ls_train_latent,
                          ...
                          self.ls_reg_loss, self.ls_div_loss, self.ls_action_loss, self.ls_average_action_var],input_list)
        ...
        self.log_latent_action_var += l11
    ...
    def get_output(self, input_state, input_route, input_latent, input_dropout=0.1):
        ...
        sess = tf.get_default_session()
        l1, l2, l3, l4 = sess.run([self.output_route_mean, self.output_route_var, self.output_action_mean, self.output_action_var], input_list)
        return l1, l2, l3, l4
    ...
    def current_log(self):
        ...
        return "\t" + "\t".join([str(self.log_latent_mean[i] / log_num) for i in range(self.latent_len)]) + "\t"\
            ...
            + str(self.log_latent_action_var / log_num) + "\t"\
    ...
    def log_print(self):
        ...
        print ( self.name \
            ...
            + "\n\tLatentActionVar     : " + str(self.log_latent_action_var / log_num) \
