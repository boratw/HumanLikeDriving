...
class DrivingStyleLearner():
    def __init__(self, name=None, reuse=False, state_len = 59, route_len = 10, nextstate_len=2, action_len=3, global_latent_len = 4,
                 ...
                 l2_regularizer_weight=0.001, fake_weight=0.01):
        if name == None:
        ...
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            self.layer_input_fake_global_latent = tf.placeholder(tf.float32, [None, global_latent_len])
            self.layer_input_fake_weight = tf.placeholder(tf.float32, [None])
            ...
            fake_latent_route_decoder_input = tf.concat([self.layer_input_fake_global_latent, self.layer_input_state, layer_input_route_selected], axis=1)
            self.fake_latent_route_decoder = MLP(global_latent_len + state_len + route_len, nextstate_len, [256, 256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=fake_latent_route_decoder_input, name="RouteDecoder", reuse=True, use_dropout=False )
            fake_latent_action_decoder_input = tf.concat([self.layer_input_fake_global_latent, self.layer_input_state, layer_input_route_flatten], axis=1)
            self.fake_latent_action_decoder = MLP(global_latent_len + state_len + route_len * action_len, action_len, [256, 256], hidden_nonlns = tf.nn.leaky_relu, 
                        input_tensor=fake_latent_action_decoder_input, name="ActionDecoder", reuse=True, use_dropout=False )
            ...
            latent_route_decoder_reconstruction_loss = tf.abs(self.layer_input_nextstate - self.latent_route_decoder.layer_output)
            latent_route_decoder_fake_loss = tf.abs(self.latent_route_decoder.layer_output - self.fake_latent_route_decoder.layer_output)
            self.latent_route_decoder_reconstruction_loss = tf.reduce_mean(latent_route_decoder_reconstruction_loss, axis=0)
            self.latent_route_decoder_fake_loss = tf.reduce_mean(latent_route_decoder_fake_loss, axis=0)
            ...
            latent_action_decoder_reconstruction_loss = tf.nn.softmax_cross_entropy_with_logits_v2(tf.one_hot(self.layer_input_action, action_len), self.latent_action_decoder.layer_output)
            latent_action_decoder_fake_loss = tf.nn.softmax_cross_entropy_with_logits_v2(tf.nn.softmax(self.latent_action_decoder.layer_output), self.fake_latent_action_decoder.layer_output)
            self.latent_action_decoder_reconstruction_loss = tf.reduce_mean(latent_action_decoder_reconstruction_loss)
            self.latent_action_decoder_fake_loss = tf.reduce_mean(latent_action_decoder_fake_loss)
    ...
    def network_initialize(self):
        ...
        self.log_fake_route_loss_rec = np.array([0.] * self.nextstate_len)
        self.log_fake_action_loss_rec = 0.
    ...
    def optimize_decoder(self, iteration_num, input_state, input_nextstate, input_route, input_action, input_global_latent, input_fake_global_latent, input_fake_weight):
        input_list = {self.layer_iteration_num : iteration_num, self.layer_input_state : input_state, self.layer_input_nextstate: input_nextstate,
                      self.layer_input_action : input_action, self.layer_input_route : input_route, self.layer_input_global_latent : input_global_latent,
                      self.layer_input_fake_global_latent : input_fake_global_latent, self.layer_input_fake_weight : input_fake_weight,  self.layer_dropout : 0.1 }
        sess = tf.get_default_session()
        _, l1, l2, l3, l4, l5, l6 = sess.run([self.decoder_train_action, 
                                              self.route_decoder_reconstruction_loss, self.action_decoder_reconstruction_loss, 
                                              self.latent_route_decoder_l2_loss, self.latent_action_decoder_l2_loss,
                                              self.latent_route_decoder_fake_loss, self.latent_action_decoder_fake_loss ],input_list)
        ...
        self.log_fake_route_loss_rec += l5
        self.log_fake_action_loss_rec += l6
    ...
    def log_caption(self):
        return "\t" + self.name + "_GlobalKLLoss\t" + self.name + "_GlobalL2Loss\t" \
            ...
            + self.name + "_FakeRouteRecLoss\t" + "\t".join([ "" for _ in range(self.nextstate_len)]) + self.name + "_FakeActionL2Loss\t" \
    ...
    def current_log(self):
        ...
        return str(self.log_encoder_loss_kl / log_encoder_num) + "\t" + str(self.log_encoder_loss_l2 / log_encoder_num) + "\t"\
            ...
            + "\t".join([str(self.log_fake_route_loss_rec[i] / log_decoder_num) for i in range(self.nextstate_len)])  + "\t"\
            + str(self.log_fake_action_loss_rec / log_decoder_num)+ "\t" \
    ...
    def log_print(self):
        ...
        print ( self.name \
            ...
            + "\n\tFakeRouteReconLoss   : " + " ".join([str(self.log_fake_route_loss_rec[i] / log_decoder_num)[:8] for i in range(self.nextstate_len)]) \
            + "\n\tFakeActionReconLoss  : " + str(self.log_fake_action_loss_rec / log_decoder_num) \
