...
def SendLatentOutput(list):
    ...
    print(cur_history[target][current_step][0][:4])
    print(cur_history[target][current_step][0][4:9])
    print(cur_history[target][current_step][0][9:14])
    print(cur_history[target][current_step][0][14:19])
    ...
    for j in range(len(res)):
        ...
        y += py
    '''
    l_state = [[cur_history[target][current_step + j][1][0], cur_history[target][current_step + j][1][1], state_vectors[current_step + j][target][2] * 0.017453293] for j in range(0, 100, 20)]
...
with sess.as_default():
    ...
    learner_saver.restore(sess, "train_log/DrivingStyle2/log_26-04-2023-15-16-50_1450.ckpt")
            ...
            for j in range(3):
                while k < len(state_vectors):
                    if len(state_vectors[k][i][8]) > 0 :
                        if np.sqrt((state_vectors[k][i][8][0][1] - prevx) ** 2 + (state_vectors[k][i][8][0][2] - prevy) ** 2) > 5.:
