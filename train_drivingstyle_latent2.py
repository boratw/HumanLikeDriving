...
from network.DrivingStyle_Latent4 import DrivingStyleLearner
...
log_name = "train_log/DrivingStyle_Latent5/log_" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
                ...
                if control_vectors[step][i][0] == 0 and control_vectors[step+4][i][0] == 0 and \
                    ...
                    if traced is not None:
                        ...
                        if action < 0:
                            ...
                            action = 30
                        history_exp[i].append( [np.concatenate([[velocity, tstate, px, py, control_vectors[step][i][1]], 
                                                                np.array(other_vcs).flatten(), np.array(route).flatten()]), prevstate, nextstate, 
                                                                np.array(param_vectors[i][:7]), action])
    return history_exp

tf.disable_eager_execution()
sess = tf.Session()
with sess.as_default():
    learner = DrivingStyleLearner(state_len=state_len, prevstate_len=prevstate_len, nextstate_len=nextstate_len, action_len=action_len, control_len=control_len,
    ...
    with multiprocessing.Pool(20) as pool:
        ...
        for epoch in range(1, 10000):
            pkl_index = random.randrange(17)
            pkl_index2 = random.randrange(4)
            print("Epoch " + str(epoch))
            if epoch % 20 == 1:
                history = history[(len(history) // 16):]
                print("Read data " + str(pkl_index) + "_" + str(pkl_index2))
                with open("data/gathered_from_npc3/data_" + str(pkl_index) + "_" + str(pkl_index2) + ".pkl","rb") as fr:
                    data = pickle.load(fr)
            ...
            for iter in range(len(history) * 4):
                ...
                for agent_index in agent_indices:
                    ...
                    control_dic.extend([cur_history[step][3] for step in step_dic])
                    action_dic.extend([cur_history[step][4] for step in step_dic])
                learner.optimize(state_dic, prevstate_dic, nextstate_dic, control_dic, action_dic)
            learner.log_print()
